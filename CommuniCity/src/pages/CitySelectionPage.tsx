    ...
    return (
      <SelectionLayout title="Select Your City">
        <div className="flex flex-col items-center justify-center h-full">
          ...
          <p className="text-xl text-gray-300">Loading cities...</p>
        </div>
      </SelectionLayout>
    );
  }
  ...
  if (error) {
    return (
      <SelectionLayout title="Select Your City">
        ...
        <Button onClick={() => window.location.reload()} className="mt-4 bg-yellow-500 hover:bg-yellow-600 text-purple-950">
          Retry
        </Button>
      </SelectionLayout>
    );
          ...
          >
            <CardHeader className="pb-2">
              ...
              <CardTitle className="text-3xl font-bold text-yellow-300 flex items-center">
                <MapPin className="mr-2 h-6 w-6 text-pink-400" />
                {city.name}
              </CardTitle>
            ...
            <CardContent className="flex-1">
              <p className="text-gray-200 text-base line-clamp-3">{city.description || 'A vibrant and bustling virtual city.'}</p>
            </CardContent>
