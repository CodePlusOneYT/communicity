    ...
    <div className="p-6 space-y-8">
          ...
          {categories.map(category => (
            <Button
              ...
              className={`
                ...
                font-semibold transition-all duration-200 rounded-full px-4 py-2
              `}
            >
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center min-h-[300px]">
          ...
          <p className="text-xl text-gray-300">Loading marketplace items...</p>
        </div>
          ...
          {filteredProducts.map((product) => (
            ...
            >
              ...
              <CardContent className="flex-1">
                <p className="text-gray-200 text-base line-clamp-3">{product.description}</p>
              </CardContent>
              <CardFooter className="pt-4 flex justify-between items-center">
                <span className="text-3xl font-extrabold text-green-300 flex items-center">
                  {product.price} <Sparkles className="ml-1 h-6 w-6 text-yellow-400" />
                </span>
                <Button
                  onClick={() => handleBuy(product.name, product.price)}
                  className="bg-yellow-500 hover:bg-yellow-600 text-purple-950 font-semibold text-lg py-2 px-4 rounded-lg transition-all duration-300"
                  disabled={loading}
                >
